...
def test_dummy_online_sync_success() -> None:
    ...
    result = provider.sync_item(entity_type="work_session", entity_id=1, operation="CREATE", payload={"id": 1})
...
def test_dummy_offline_preserves_data() -> None:
    ...
    result = provider.sync_item(entity_type="work_session", entity_id=1, operation="CREATE", payload={})
    ...
    result2 = provider.upload_screenshot(screenshot_id=1, file_path="/tmp/x.jpg", metadata={})
...
def test_dummy_backend_unavailable() -> None:
    ...
    result = provider.sync_item(entity_type="break", entity_id=2, operation="CREATE", payload={})
    assert result.success is False
    assert result.retryable is True
...
def test_dummy_auth_required_non_retryable() -> None:
    ...
    result = provider.sync_item(entity_type="user", entity_id=1, operation="CREATE", payload={})
    assert result.success is False
...
def test_dummy_injected_failures_then_success() -> None:
    ...
    assert provider.sync_item(entity_type="work_session", entity_id=1, operation="CREATE", payload={}).success is False
    assert provider.sync_item(entity_type="work_session", entity_id=1, operation="CREATE", payload={}).success is False
    # Third should succeed
    assert provider.sync_item(entity_type="work_session", entity_id=1, operation="CREATE", payload={}).success is True
    assert len(provider.synced_items) == 1
...
def test_dummy_screenshot_validates_file(tmp_path: Path) -> None:
    ...
    result = provider.upload_screenshot(screenshot_id=10, file_path=missing, metadata={})
    assert result.success is False
    ...
    empty.write_bytes(b"")
    result2 = provider.upload_screenshot(screenshot_id=11, file_path=str(empty), metadata={})
    assert result2.success is False
    ...
    result3 = provider.upload_screenshot(screenshot_id=12, file_path=str(valid), metadata={"checksum": "abc"})
...
def test_sync_result_helpers() -> None:
    ...
    assert SyncResult.auth_required().connectivity == ConnectivityState.AUTHENTICATION_REQUIRED
