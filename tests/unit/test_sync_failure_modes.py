...
def test_corrupt_screenshot_file_handling(tmp_path: Path) -> None:
    ...
    try:
        ...
        from app.infrastructure.database.repositories import ScreenshotRepository, UserRepository, SessionRepository

        with db.session() as s:
            u_repo = UserRepository(s)
            user = u_repo.upsert(external_user_id="ext", email="a@example.com", display_name="A", team_name="T")
            s.flush()
        ...
        with db.session() as s:
            ...
            shot = repo.add(session_id=ws_id, captured_at=T0, activity_state=ActivityState.ACTIVE, file_path=str(missing))
            s.commit()
            shot_id = shot.id
        ...
        assert result["failed"] == 1
        with db.session() as s:
            row = s.get(SyncQueueItem, shot_id)  # queue may be empty, check screenshot state
...
def test_cleanup_handles_missing_file_gracefully(tmp_path: Path) -> None:
    ...
    try:
        ...
        with db.session() as s:
            user = UserRepository(s).upsert(external_user_id="ext", email="a@example.com", display_name="A", team_name="T")
            s.flush()
            ws = SessionRepository(s).create(user_id=user.id, started_at=T0)
            s.commit()
            ws_id = ws.id
        ...
        with db.session() as s:
            repo = ScreenshotRepository(s)
            shot = repo.add(session_id=ws_id, captured_at=T0, activity_state=ActivityState.ACTIVE, file_path=str(ghost))
