...
Builds the application QSS from :data:`app.ui.theme.tokens.DARK`. Object
...
def build_dark_stylesheet(p: Palette) -> str:
    """Return the full dark-theme QSS for palette ``p``."""
    return f"""
* {{
    font-family: {_FAMILY};
    font-size: 14px;
...
QPushButton[role="danger"]:hover {{ background: rgba(239, 68, 68, 0.24); }}
...
QPushButton[role="ghost"]:disabled {{ color: rgba(154, 163, 178, 0.45); }}
...
QPushButton#PrimaryCta:focus {{ border: 1px solid #FFFFFF; }}
...
QLineEdit:disabled {{ color: rgba(154, 163, 178, 0.60); background: {p.surface}; }}
...
QMenu::item:disabled {{ color: rgba(154, 163, 178, 0.55); background: transparent; }}
QMenu::separator {{
    height: 1px;
    background: {p.border};
    margin: 6px 8px;
